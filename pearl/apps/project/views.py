...
from .forms import ProjectForm, StartProcessingForm
from .models import MeshDiseases, MeshTissues, Project, STATUS_CODES
from apps.accounts.models import Customer, Discount
from pearl.settings.base import PROJECTS_URL
...
class QcReportView(FormView):
    ...
    def get_context_data(self,  **kwargs):
        ...
        if project.status >= 15:
            ...
            context['vcf_link'] = os.path.join(PROJECTS_URL, str(project_id), vcf)
            csv = 'ctog' + str(project_id).zfill(6) + '_report' + '.csv'
            context['csv_link'] = os.path.join(PROJECTS_URL, str(project_id), csv)
